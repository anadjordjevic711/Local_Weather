...
function sevDaysWeather (seven) {
  $.ajax({
    ...
    success: function(seven) {
      ...
      $("#sevDaysTable").append('<tr> <td>' + (i+1) + '. day </td> <td> <img src=http://openweathermap.org/img/w/' + sevDays[i].weather[0].icon + '.png> </td> <td>' + sevDays[i].temp.min.toFixed(1) + '<span style="color: rgba(0,0,0,0);">l</span>' + unitsSymbol + '</td> <td>' + sevDays[i].temp.max.toFixed(1) + '<span style="color: rgba(0,0,0,0);">l</span>' + unitsSymbol +'</td> </tr>');
      }
    }
  });
}
